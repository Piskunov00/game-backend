...
from api.utils.base import http_wrapper, get_user
...
def sign_up(request):
    data = request.POST
    if Gamer.objects.filter(name=data['name']).count():
        return HttpResponse(
    ...
    cur_player = Gamer(  # todo: добавить форму валидации пользователя
        name=data['name'],
    ...
    cur_player.password = make_password(data['password'])
    cur_player.save()
