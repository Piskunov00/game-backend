...
def test_that_check_view_sign_up(rf):
    ...
    new_user = Gamer.objects.get(user__username='test')
    assert 'sha256$' in new_user.user.password
...
def test_create_game(rf, tester):
    ...
    with patch('tmp_name.business.actions.choice', return_value=5432):
...
def test_that_check_that_205_if_there_are_many_games(rf, tester):
    import tmp_name.business.actions
    with patch('tmp_name.business.actions.choice', side_effect=IndexError()):
        request = rf.post('/create_game/', data=dict(type=1))
        request.user = tester
...
@pytest.mark.django_db
@patch('tmp_name.business.actions.choice', return_value=7777)
@patch('tmp_name.business.actions.time', return_value=10)
